...
Attention à l'intervalle de confiance : si on est dans l'intervalle, c'est sûr que l'échantillon est bon. Si on n'est pas dans l'intervalle, alors on peut se douter d'un problème mais ce n'est pas une preuve de non-conformité.
...
\subsection{Est-ce que la moyenne correspond à l'intervalle de confiance ?}

On prend comme exemple ``la taille des français'' avec les valeurs suivantes : $n=100$ la taille de l'échantillon, et une probabilité $p$ ; c'est donc une loi binomiale.

On cherche ensuite l'écart-type de cette loi : \[\sigma_{\overline{X}} = \frac{\sqrt{np(1-p)}}{n} = \sqrt{\frac{p(1-p)}{n}}\]

On trouve ensuite l'intervalle de confiance : $\overline{X} \pm u_\alpha \times \sigma_{\overline{X}}$

Quand on cherche un intervalle de confiance à $\alpha$ pourcents, on utilise la loi normale pour trouver la valeur correspondant, c'est $u_\alpha$ (eg. Pour $\alpha=0.95$, on prend l'inverse, $0.05$, et on le cherche dans la table de la loi normale, ce qui donne $u_\alpha=1.96$).

On peut donc finir le calcul et décider si la moyenne trouvée est dans l'intervalle de confiance.

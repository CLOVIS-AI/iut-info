...
En droit français, le contrat est dit consensuel (il relève du consentement) : à chaque fois que l'on échange avec quelqu'un, on forme un contrat. La forme principale est donc la\mainentry{Contrat!oral} forme orale.
...
Un enregistrement vocal ou vidéo compte comme un ``commencement de\entry{Preuve!Commencement de} preuve'': le juge a un pouvoir d'interprétation et peut annuler le contrat.
...
Les données\entry{Données!personnelles} personnelles doivent respecter la RGDP (au contraire des données\entry{Données!techniques} techniques, qui peuvent être utilisées librement).
...
\subsubsection{Principes}\entry{Principe}

\paragraph{Principe de Finalité}\mainentry{Principe!de finalité}
...
\paragraph{Principe de proportionnalité et pertinence des données}\mainentry{Principe!de proportionnalité}\mainentry{Principe!de pertinence des données}
...
\paragraph{Principe de conservation limitée}\mainentry{Principe!de conservation limitée}
...
\paragraph{Principe de sécurité et de confidentialité}\mainentry{Principe!de sécurité et confidentialité}
...
\paragraph{Droit des personnes}\mainentry{Droit!des personnes}
...
Le RGDP\entry{RGDP} s'applique aux entreprises situées en Europe, ou dont les utilisateurs sont situés en Europe.

\subsubsection{Conformité et non-conformité}

Si l'on est pas conforme, on peut être sanctionné par la\entry{CNIL} CNIL:
\begin{enumerate}
\item Avertissement,
...
N'importe quel citoyen peut demander une enquête de la\entry{CNIL} CNIL.

\subsubsection{Définition des données personnelles}

\begin{cquote}{Donnée\mainentry{Données!personnelles} personnelle}
...
Le RGDP reprend les principes de la loi précédente \cf{sec:loi-info-liberte}. Il y ajoute d'autres principes:

\paragraph{Principe de minimisation}\mainentry{Principe!de minimisation}
...
\paragraph{Principe de conformité}\mainentry{Principe!de conformité}
...
\subsubsection{Données sensibles}\mainentry{Données!personnelles!sensibles}

Les données qui révèlent l'origine ethnique, les opinions politiques, philosophiques, religieuses, l'appartenance syndicale, les données de santé, l'orientation sexuelle, données génétiques ou biométriques, sont considérées comme critiques. Il est obligatoire dans ce cas de nommer un\mainentry{DPO} DPO (Data Protecter Officer, un responsable des données), qui doit s'assurer de la sécurité des lois.
...
Quand une faille est détectée, il faut la transmettre à la CNIL\entry{CNIL} dans les 72 heures (sur leur site).
